    ...
    <div className="space-y-6 relative z-10 text-slate-100 font-sans">
      ...
      <div className="flex items-center gap-4 bg-slate-950/40 border border-slate-900/50 p-1.5 rounded-xl self-start w-fit relative z-40">
        <button
          ...
          className={`px-4 py-2 rounded-lg font-mono text-[9px] font-black uppercase tracking-widest transition-all duration-200 cursor-pointer ${activeTab === 'market'
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg font-mono text-[9px] font-black uppercase tracking-widest transition-all duration-200 cursor-pointer ${activeTab === 'technical'
              ? 'bg-neon-cyan/10 border border-neon-cyan/30 text-neon-cyan shadow-[0_0_12px_rgba(0,240,255,0.15)]'
              : 'bg-transparent text-slate-500 hover:text-slate-350 border border-transparent'
            }`}
        >
            ...
            {(indices || []).map((idx, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`glass-card rounded-2xl p-4 flex flex-col justify-between border relative overflow-hidden group transition-all duration-300 ${isUpIdx ? 'border-neon-emerald/10 hover:border-neon-emerald/30 shadow-[0_4px_20px_rgba(0,255,102,0.04)]' : 'border-neon-rose/10 hover:border-neon-rose/30 shadow-[0_4px_20px_rgba(255,45,85,0.04)]'
                    }`}
                >
            ...
            <div className="lg:col-span-2 glass-card rounded-2xl p-5 border border-slate-800/60 relative overflow-hidden flex flex-col justify-between">
              ...
              <div>
                <div className="flex justify-between items-center mb-4 border-b border-slate-900/50 pb-3">
                  ...
                  <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${marketSummary.sentiment.includes('Positive')
                ...
                <div className="space-y-2 overflow-y-auto max-h-56 pr-1 custom-scrollbar">
                  {(notifications || []).map(notif => (
                    <div
                      ...
                      className={`p-2.5 rounded-xl border font-mono text-[8px] transition-all duration-200 ${notif.isRead
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mt-5 pt-4 border-t border-slate-900/50 font-mono">
                          <div>
                            ...
                            <span className={`text-sm sm:text-base font-black tracking-tight ${isBullishTrend ? 'text-neon-emerald' : 'text-neon-rose'}`}>
                              {chartMeta ? formatCurrency(chartMeta.currentPrice, currentStock.currency) : formatCurrency(predictionData?.stock?.currentPrice || price, currentStock.currency)}
                            </span>
                          </div>
                          <div>
                        ...
                        <div className="flex flex-col gap-4 mb-5">
                          <div className="flex items-baseline justify-between">
                            ...
                            <div className="text-right">
                              ...
                              <span className={`px-4 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest font-mono inline-block ${activePrediction.prediction === 'BUY' ? 'glow-badge-buy' :
        ...
        <AnimatePresence>
          {toasts.map(toast => (
            <motion.div
              ...
              className={`pointer-events-auto p-4 rounded-xl border flex flex-col shadow-2xl relative overflow-hidden backdrop-blur-xl ${toast.type === 'success' ? 'bg-[#00ff66]/10 border-[#00ff66]/40 text-slate-100 shadow-[0_0_20px_rgba(0,255,102,0.15)]' :
